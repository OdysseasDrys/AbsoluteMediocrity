...
def get_max_instance(instance_candidates, contrast_pred_idx):
    ...
    batched_preds = predictor.predict_batch_instance(instance_candidates)
    batched_preds = [add_prob(pred) for pred in batched_preds]
    max_idx = min(range(len(batched_preds)), key=lambda index: abs(batched_preds[index]['probs'][contrast_pred_idx] - 0.5))
    max_candidate = instance_candidates[max_idx]
    max_prob = batched_preds[max_idx]['probs'][contrast_pred_idx]
...
class EditEvaluator():
    ...
    def score_fluency(self, sent):
        ...
        avg_loss = sm(temp_losses)/len(temp_losses)
        return avg_loss
...
def sort_instances_by_score(scores, *args):
    """ Sorts *args in order of increasing absolute differences from 0.5 """

    # Calculate absolute differences from 0.5 for each score
    abs_diff = [abs(score - 0.5) for score in scores]

    # Zip the scores, absolute differences, and *args
    zipped = list(zip(abs_diff, scores, *args))

    # Sort based on increasing absolute differences
    zipped.sort()

    # Remove absolute differences after sorting and return the sorted list
    sorted_list = [list(elem)[1:] for elem in zipped]
...
def get_scores(predictor, instance_candidates, contrast_pred_idx, k = None):
    ...
    if k != None:
        mid_probs =  torch.abs(probs-0.5)
        pred_indices = torch.argmin(mid_probs, dim=1)
        ...
        sorted_contrast_probs = torch.argsort(contrast_probs, descending=False)
        ...
        cpu_pred_indices = torch.argmin(mid_probs, dim=1).cpu().numpy()
...
class EditFinder():
    ...
    def run_edit_round(
        ...
        logger.info(wrap_text(f"Running candidate generation for mask frac:{mask_frac}; max mask frac: {self.max_mask_frac}"))
        self.editor.masker.mask_frac = mask_frac

        # Get candidates form editor
        ...
        #Get the scores for all candidates
        # we want mid indices not highest
        # We get the probabilities. the indices of the probs and the indices with the highest probs
        # we want to retrieve the indices of the middle probs

        probs, pred_indices, mid_indices = get_scores(self.predictor, 
                instance_cands, contrast_pred_idx, k = self.beam_width)

        # get the right candidates based on the highest indices

        input_cands = [input_cands[idx] for idx in mid_indices]
        editable_seg_cands = [editable_seg_cands[idx] for idx in mid_indices]
        
        # Sort these by highest score for iteration
        # when sorting we want to sort by ascending distance from 0.5
        ...
        for sort_idx, (prob, pred_idx, input_cand, editable_seg_cand) in iterator:
            if self.verbose and sort_idx == 0:
                print()
                logger.info(wrap_text(f"Post edit round, prob closest to 0.5: {prob} with mask frac: {mask_frac}"))
                print()
                logger.info(wrap_text(f"Post edit round, best candidate: {input_cand}"))
                print()
            ...
            if prob>0.4 and prob<0.6:
                ...
                with open('candidates.txt', 'a') as file:
                    file.write(f"\nPost edit round, prob closest to 0.5: {prob} with mask frac: {mask_frac}\n{input_cand}\n")
    ...
    def binary_search_edit(
            ...
            min_mask_frac=0.3, max_mask_frac=0.6, num_levels=1, 
    ...
    def linear_search_edit(
            self, edit_list, input_cand, contrast_pred_idx, num_rounds, 
            min_mask_frac=0.3, max_mask_frac=0.6, max_levels=None, 
